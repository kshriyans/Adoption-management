...
/*if(isset($_SESSION["username"]))
{
	if($_SESSION["usertype"] == 1)
		header('Location: /Adoption/public/adopter/');

	if($_SESSION["usertype"] == 2)
		header('Location: /Adoption/public/orphanage/');
	    ...
	    if(mysqli_num_rows($rs) > 0)
	    {
			if($row = mysqli_fetch_array($rs, MYSQLI_ASSOC))
			{
				...
				if($passwd == $hashedpasswd)
				{
					...
					if($usertype == 1)
	                	$redirect_url = "Location: http://" . $_SERVER["HTTP_HOST"] . "/Adoption/public/adopter/profileinit.php";
					elseif($usertype == 2)
						$redirect_url = "Location: http://" . $_SERVER["HTTP_HOST"] . "/Adoption/public/orphanage/profileinit.php";
	                header($redirect_url);
	                exit;
				}
				else
